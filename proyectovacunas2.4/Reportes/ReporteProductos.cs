...
namespace proyectovacunas2._4.Reportes
{
    public partial class ReporteProductos : Form
    {
        ...
        public void GenerarPDF(DataGridView dtProducto)
        {
                ...
                using (Document doc = new Document(PageSize.LETTER, 36, 36, 36, 36))
                {
                    try
                    {
                        ...
                        using (PdfWriter writer = PdfWriter.GetInstance(doc, new FileStream(rutaDestino, FileMode.Create)))
                        {
                            ...
                            // Título de la tabla
                            Paragraph tableTitle = new Paragraph("Reporte de Productos", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 16, iTextSharp.text.Font.BOLD, colorTitulo));
                            ...
                            // Tabla de datos
                            PdfPTable dataTable = new PdfPTable(dtProducto.Columns.Count);
                            dataTable.WidthPercentage = 100;

                            // Agregar encabezados de columna
                            for (int i = 0; i < dtProducto.Columns.Count; i++)
                            {
                                PdfPCell cell = new PdfPCell(new Phrase(dtProducto.Columns[i].HeaderText, new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, iTextSharp.text.Font.BOLD, colorTexto)));
                            ...
                            {
                                for (int j = 0; j < dtProducto.Columns.Count; j++)
                                {
                                    PdfPCell cell = new PdfPCell(new Phrase((dtProducto.Rows[i].Cells[j].Value ?? "").ToString(), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10, iTextSharp.text.Font.NORMAL, colorTexto)));
                                    cell.HorizontalAlignment = Element.ALIGN_CENTER;
                                    dataTable.AddCell(cell);
                                }
                            }
        ...
        public void GenerarExcel(DataGridView dtProducto)
        {
            // Validar que la DataGridView no esté vacía
            if (dtProducto.Rows.Count == 0)
            {
            ...
            saveFileDialog.FileName = "TablaProductos.xlsx";
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                using (ExcelPackage excelPackage = new ExcelPackage(new FileInfo(rutaDestino)))
                {
                    try
                    {
                        ...
                        ExcelWorksheet worksheet = excelPackage.Workbook.Worksheets.Add("Reporte de Productos");

                        // Encabezados de columna
                        for (int i = 0; i < dtProducto.Columns.Count; i++)
                        {
                            worksheet.Cells[1, i + 1].Value = dtProducto.Columns[i].HeaderText;
                        ...
                        for (int i = 0; i < dtProducto.Rows.Count; i++)
                        {
                            for (int j = 0; j < dtProducto.Columns.Count; j++)
                            {
                                worksheet.Cells[i + 2, j + 1].Value = (dtProducto.Rows[i].Cells[j].Value ?? "").ToString();
