...
namespace proyectovacunas2._4.Reportes
{
...

    {
        ...
        public void GenerarPDF(DataGridView dtEmpleado)
        {
            // Crear un diálogo de guardar archivo
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.Filter = "Archivos PDF|*.pdf";
            saveFileDialog.Title = "Guardar archivo PDF";
            saveFileDialog.FileName = "ReporteEmpleados.pdf";
                ...
                using (Document doc = new Document(PageSize.LETTER, 36, 36, 36, 36))
                {
                    try
                    {
                        ...
                        using (PdfWriter writer = PdfWriter.GetInstance(doc, new FileStream(rutaDestino, FileMode.Create)))
                        {
                            ...
                            // Título de la tabla
                            Paragraph tableTitle = new Paragraph("Reporte de Empleados", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 16, iTextSharp.text.Font.BOLD, colorTitulo));
                            ...
                            // Tabla de datos
                            PdfPTable dataTable = new PdfPTable(dtEmpleado.Columns.Count);
                            dataTable.WidthPercentage = 100;

                            // Agregar encabezados de columna
                            for (int i = 0; i < dtEmpleado.Columns.Count; i++)
                            {
                                PdfPCell cell = new PdfPCell(new Phrase(dtEmpleado.Columns[i].HeaderText, new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, iTextSharp.text.Font.BOLD, colorTexto)));
                            ...
                            {
                                for (int j = 0; j < dtEmpleado.Columns.Count; j++)
                                {
                                    PdfPCell cell = new PdfPCell(new Phrase((dtEmpleado.Rows[i].Cells[j].Value ?? "").ToString(), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10, iTextSharp.text.Font.NORMAL, colorTexto)));
                                    cell.HorizontalAlignment = Element.ALIGN_CENTER;
                                    dataTable.AddCell(cell);
                                }
                            }
        ...
        public void GenerarExcel(DataGridView dtEmpleado)
        {
            // Crear un diálogo de guardar archivo
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.Filter = "Archivos de Excel|*.xlsx";
            saveFileDialog.Title = "Guardar archivo de Excel";
            saveFileDialog.FileName = "TablaEmpleados.xlsx";
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                using (ExcelPackage excelPackage = new ExcelPackage(new FileInfo(rutaDestino)))
                {
                    try
                    {
                        ...
                        ExcelWorksheet worksheet = excelPackage.Workbook.Worksheets.Add("Reporte de Empleados");

                        // Encabezados de columna
                        for (int i = 0; i < dtEmpleado.Columns.Count; i++)
                        {
                            worksheet.Cells[1, i + 1].Value = dtEmpleado.Columns[i].HeaderText;
                        ...
                        for (int i = 0; i < dtEmpleado.Rows.Count; i++)
                        {
                            for (int j = 0; j < dtEmpleado.Columns.Count; j++)
                            {
                                worksheet.Cells[i + 2, j + 1].Value = (dtEmpleado.Rows[i].Cells[j].Value ?? "").ToString();
